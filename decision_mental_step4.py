...
for num_features in range(1, 11):
    ...
    for folds in [3, 5]:  # Number of folds for validation
        ...
        for train_index, val_index in kf.split(X_train_selected):
            ...
            y_pred_fold = dt_classifier.predict(X_val_fold)
...
# Select the top 5 combinations of features and folds
top_results = sorted_results.groupby(['num_features', 'folds']).head(1).head(5)

# Display the top 5 combinations
print("Top 5 Combinations of Features and Folds:")
print(top_results[['num_features', 'folds', 'Accuracy']])

# Train a Decision Tree Classifier using all features
dt_classifier_all = DecisionTreeClassifier(max_depth=None)
dt_classifier_all.fit(X_train, y_train)

# Plot the decision tree with feature labels
plt.figure(figsize=(8, 5))
plot_tree(dt_classifier_all, filled=True, feature_names=X.columns, class_names=['No', 'Yes'], fontsize=10)
plt.show()


# Save results to Excel
results_df.to_excel("D:/Sem4/ML/Project/Results/decision_tree_results1.xlsx", index=False)
